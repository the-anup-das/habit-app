import { PopulatedEntry } from "./basic";
...
export function calculateInfluenceOnMood(entries: PopulatedEntry[]): ActivityInfluence[] {
  ...
  const getPrevDay = (d: number) => {
    ...
    const date = new Date(parseInt(dStr.slice(0,4)), parseInt(dStr.slice(4,6)) - 1, parseInt(dStr.slice(6,8)));
  ...
  for (const [actId, actName] of allActivities.entries()) {
    ...
    const ePlus = entries.filter(e => e.activities.some(a => a.activityId === actId)).map(e => e.mood.score);
    const eMinus = entries.filter(e => !e.activities.some(a => a.activityId === actId)).map(e => e.mood.score);
    const withWithout = mean(ePlus) - mean(eMinus);

    // Days with/without
    const daysWithAct = new Set(entries.filter(e => e.activities.some(a => a.activityId === actId)).map(e => e.localDate));
    ...
    // Prev day
    const prevDaysPlus = Array.from(daysWithAct).map(d => getPrevDay(d)).filter(d => avgMoodByDay.has(d)).map(d => avgMoodByDay.get(d)!);
    const prevDaysMinus = allDays.filter(d => !daysWithAct.has(d)).map(d => getPrevDay(d)).filter(d => avgMoodByDay.has(d)).map(d => avgMoodByDay.get(d)!);
    const prevDayDiff = mean(prevDaysPlus) - mean(prevDaysMinus);

    // Next day
    const nextDaysPlus = Array.from(daysWithAct).map(d => getNextDay(d)).filter(d => avgMoodByDay.has(d)).map(d => avgMoodByDay.get(d)!);
    const nextDaysMinus = allDays.filter(d => !daysWithAct.has(d)).map(d => getNextDay(d)).filter(d => avgMoodByDay.has(d)).map(d => avgMoodByDay.get(d)!);
    ...
    if (nPlus > 1 && nMinus > 1) {
      ...
      const dfNum = Math.pow((varPlus / nPlus) + (varMinus / nMinus), 2);
      const dfDenom = Math.pow(varPlus / nPlus, 2) / (nPlus - 1) + Math.pow(varMinus / nMinus, 2) / (nMinus - 1);
      ...
      entries.filter(e => e.activities.some(a => a.activityId === actId)).forEach(e => {
        const others = e.activities.map(a => a.activityId).filter(id => id !== actId).sort().join(",");
        combos.add(others);
      });
