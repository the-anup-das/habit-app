...
export async function deriveRowKey(masterKey: Uint8Array, context: string = "rows"): Promise<Uint8Array> {
  await initCrypto();
...
export async function deriveFileKey(masterKey: Uint8Array, context: string = "files"): Promise<Uint8Array> {
  await initCrypto();
  let ctx = new TextEncoder().encode(context);
  if (ctx.length < 8) {
    const padded = new Uint8Array(8);
