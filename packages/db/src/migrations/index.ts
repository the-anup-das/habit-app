// Generated by drizzle-kit, inlined so both apps can run migrations without
// a bundler-specific SQL loader. Regenerate with:
//   pnpm --filter @chapter/db exec drizzle-kit generate
export const MIGRATIONS: readonly string[] = [
  ...
  `CREATE INDEX \`idx_entries_mood\` ON \`entries\` (\`mood_id\`);`,
  ...
  `CREATE INDEX \`idx_ea_activity\` ON \`entry_activities\` (\`activity_id\`);`,
  ...
  `CREATE UNIQUE INDEX \`idx_checkin_unique\` ON \`goal_checkins\` (\`goal_id\`,\`local_date\`);`,
  ...
  `CREATE INDEX \`idx_attempts_tracker\` ON \`quit_attempts\` (\`tracker_id\`,\`started_at\`);`,
  ...
  `CREATE INDEX \`idx_activities_group\` ON \`activities\` (\`group_id\`,\`position\`);`,
  ...
  `CREATE INDEX \`idx_moods_group\` ON \`moods\` (\`group_id\`,\`position\`);`,
  `CREATE TABLE \`scales\` (
	\`id\` text PRIMARY KEY NOT NULL,
	\`created_at\` integer NOT NULL,
	\`updated_at\` integer NOT NULL,
