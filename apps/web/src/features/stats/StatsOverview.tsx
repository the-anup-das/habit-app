...
export function StatsOverview() {
  ...
  if (loading) {
    return <div style={{ padding: "2rem", textAlign: "center", color: "var(--color-ink-3)" }}>Loading stats...</div>;
  }
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: "48rem", margin: "0 auto", paddingBottom: "6rem" }}>
      ...
      <section style={{ marginBottom: "2rem" }} ref={chartRef}>
        ...
        <div className="glass-panel" style={{ padding: "1rem", borderRadius: "var(--radius-xl)", overflowX: "auto" }}>
          {chartData ? (
            <svg width="100%" height={240} viewBox={`0 -20 ${chartRef.current?.clientWidth || 600} 240`} style={{ overflow: "visible" }}>
              <path 
                d={chartData.path} 
                fill="none" 
            ...
            <div style={{ padding: "2rem", textAlign: "center", color: "var(--color-ink-3)" }}>Not enough data for chart.</div>
          )}
        </div>
      </section>
      ...
      <section style={{ marginBottom: "2rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {influenceData.length > 0 ? influenceData.map(inf => {
            const isHigh = inf.confidence === "HIGH";
            const isMedium = inf.confidence === "MEDIUM";
            const isLow = inf.confidence === "LOW";
            return (
              <div key={inf.activityId} className="glass-panel" style={{ padding: "1.5rem", borderRadius: "var(--radius-xl)", display: "flex", justifyContent: "space-between", alignItems: "center", opacity: isLow ? 0.6 : 1 }}>
                <div>
                  <div style={{ fontWeight: "600", fontSize: "var(--font-size-lg)" }}>{inf.activityName}</div>
                  <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-ink-3)", marginTop: "0.25rem" }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "1.5rem", fontWeight: "bold", color: isLow ? "var(--color-ink-3)" : inf.isPositive ? "#10b981" : "#ef4444" }}>
                    {inf.sameDay > 0 ? "+" : ""}{inf.sameDay.toFixed(2)}
                  </div>
                  <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-ink-3)" }}>Same Day</div>
                </div>
              </div>
            );
          }) : (
            <div style={{ padding: "2rem", textAlign: "center", color: "var(--color-ink-3)", fontStyle: "italic" }}>
      ...
      {/* Streak Panel */}
      <section style={{ marginBottom: "2rem" }}>
        <div className="glass-panel" style={{ padding: "2rem", borderRadius: "var(--radius-xl)", display: "flex", justifyContent: "space-around", textAlign: "center" }}>
          <div>
            <div style={{ fontSize: "3rem", fontWeight: "bold", color: "var(--color-primary)" }}>{streak.current}</div>
            <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-ink-3)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Current Streak</div>
          </div>
          <div>
            <div style={{ fontSize: "3rem", fontWeight: "bold", color: "var(--color-ink-1)" }}>{streak.longest}</div>
            <div style={{ fontSize: "var(--font-size-sm)", color: "var(--color-ink-3)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Longest Streak</div>
          </div>
        </div>
      </section>
      ...
      >
        <Link to="/" style={{ padding: "0.75rem 1.5rem", borderRadius: "var(--radius-full)", textDecoration: "none", color: "var(--color-ink-2)", fontWeight: "500" }}>
          Timeline
        </Link>
        <Link to="/stats" style={{ padding: "0.75rem 1.5rem", borderRadius: "var(--radius-full)", textDecoration: "none", color: "white", background: "var(--color-primary)", fontWeight: "600", boxShadow: "var(--shadow-md)" }}>
          Stats
        </Link>
      </div>
    </div>
